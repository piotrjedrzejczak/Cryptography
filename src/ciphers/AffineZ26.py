...
class AffineZ26(Cipher):
    ...
    def encrypt(cls, text, keys):
        key1, key2 = keys
        if (
            key1 in cls.__inverses.keys() and
            key2 in cls.__inverted_alphabet.keys()
        ):
            encrypted = ""
            charset = set(cls.__alphabet.keys())
            for char in text:
                if char in charset:
                    encrypted += cls.__inverted_alphabet[
    ...
    def decrypt(cls, text, keys):
        ...
        if (
            ...
            for char in text:
                if char in charset:
                    decrypted += cls.__inverted_alphabet[
                        (cls.__inverses[key1]) *
    ...
    def cryptoanalysis(cls, encrypted, plain):
        for key1 in cls.__inverses.keys():
            for key2 in cls.__inverted_alphabet.keys():
                decrypted = cls.decrypt(encrypted, (key1, key2))
